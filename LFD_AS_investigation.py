...
uptake = 0.4
cohort_sizes = np.arange(0.,1.1,0.1)
cohort_test_freq = [7,4,2] # once a week, twice a week, every 2 days
...
pcr_symptomatic_testing = test_prob_with_curve(test_profile_key = "lfa",
                                                symp_prob = uptake,
                                                asymp_prob = 0.)

ct = CT_no_quarantine(trace_probs = trace_probs,
...
asympt_testing = {f"{c:.2f}_{cohort_test_freq[i]}":cohort_testing(test_profile_key = "lfa",
                                 anti_size = 0.1,
                                 cohort_size = c,
                                 test_freq = cohort_test_freq[i],
                                 test_prob = remaining_test_prob[i]) for c in cohort_sizes for i in range(len(cohort_test_freq))}

ili_ST_iso = ili_single_test(ili_prev = 0.01, adherence = uptake)
...
interventions = {k: [AZ_high, pfizer_high, ili_ST_iso, pcr_symptomatic_testing,asympt_testing[k], ct] for k in asympt_testing}
...
def run_msim(sim_label, contact_scenario, growth_rate):
    ...
    msim.mean()
    msim.save(f"asympt_testing/LFD_ASYMPT_INVESTIGATION_{contact_scenario}_{growth_rate}_{sim_label}.msim")
...
for contact_scenario in ["September2020", "Polymod"]:
