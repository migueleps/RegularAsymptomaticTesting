...
edge_cutting = {f"{r:.2f}":cv.clip_edges(0,r,layers=["c","s","w"]) for r in edge_reduction}
...
interventions = {k: [AZ_high, pfizer_high, ili_ST_iso, pcr_symptomatic_testing,edge_cutting[k], ct] for k in edge_cutting}
...
def run_msim(sim_label, contact_scenario, growth_rate):
    ...
    msim.mean()
    msim.save(f"edgecut/LFD_EDGECUT_INVESTIGATION_{contact_scenario}_{growth_rate}_{sim_label}.msim")
